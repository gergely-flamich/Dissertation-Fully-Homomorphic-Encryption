...
\begin{proof}
  ...
  \begin{algorithmic}
    ...
    \State
  \end{algorithmic}
  Since $\Enc$ is deterministic, $\A$ will guess correctly with probability 1,
...
\begin{proof}
  ...
  $\Pi$ is IND-atk secure, but by Theorem \ref{thm:determ_enc_insecure} we know
  that $\Pi$ cannot be IND-atk secure, a contradiction, therefore our initial
...
unrealistic to only consider this simple case. Luckily, it can be shown that \textbf{goal-atk}
security under a single encryption implies \textbf{goal-atk} security under
...
Section 11 in \ref{katz2014introduction} (in particular Theorem 11.6).
