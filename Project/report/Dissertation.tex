...
\paragraph{}
Cryptography is a fascinating subject, not only because it occupies the grey
...
science through the works of Shannon \cite{shannon1949communication}, later Diffie and Hellman
...
that they do not only allow encrypting data, but also arbitrary computation to
take place on the encrypted data. At first, this might not even sound possible -
after all, how could we ensure that if we manipulate encrypted data, the result
will correct at all, let alone be allowed to do as many manipulations as we
wish? The aim of this work is to provide an answer to this question.
\paragraph{}
...
most of the groups we will be dealing with in this work will indeed be finite.
A very natural operation on an arbitrary group element is to multiply it by
itself some $n$ times. So, for some group $G$ and $g \in G$, we will refer to
\[
  \underbrace{g\cdot g \cdots g}_\text{$n$ times} = g^n
\]
as the $n$-th power of $g$.
\begin{definition}{Big-O notation:} Let $f, g: \Nat \mapsto \Reals $ be
  ...
  \[
    ...
    |f(n)| \leq M|g(n)|.
  \]
  Intuitively, this means that $f$ grows \textbf{at most} as fast as $g$ as the
  input gets big.
\end{definition}
\section{A Very Brief History of Ciphers}
\paragraph{}
One of the earliest known ciphers goes back at least 2000 years, to Roman times. We will now examine the
cipher now named after Julius Ceasar, who used the following method to obfuscate
his correspondence for his adversaries:
...
\[
  \textit{Zhqm, zmgm, zmfm} \quad\to\quad \textit{Veni, vidi, vici}.
\]
\paragraph{Note:} The above is to show an example of the shifting property. In
practice the punctuation and spaces between words would also have been removed \cite{ceasarCipher}.
\paragraph{}
...
\begin{definition}{Cipher:}
  ...
  A cipher (also called crypto scheme or cryptosystem) $\Pi$ over $\M$, $\K_\Enc$ and $\K_\Dec$ is a tuple $(\Gen, \Enc, \Dec)$, where
  \begin{itemize}
  \item $\Gen$ is the \textbf{key generation algorithm}, which outputs a key
    ...
    \textbf{key space} of $\Pi$. If for all outputs there is a polynomial time
    function $f$ such that $f(k_\Enc) = k_\Dec$, then we call $\Pi$
    a \textbf{symmetric cipher} and instead of the tuple we just write $k$.
    Otherwise, we call $\Pi$ an \textbf{asymmetric cipher}.
  ...
  \item $\Enc: \K_\Enc^*\times\M\mapsto \C$ is the \textbf{encryption algorithm}, which takes
    an encryption key $k$ and a message $m$ and outputs its encoding $c$. We will often
  ...
  \end{itemize}
  Finally, we require the \textit{correctness} of $\Pi$, concretely
  \[
...
English (or Latin) alphabet. For modern cryptosystems we will always work with
...
\paragraph{Example}
We can rephrase the definitions of symmetric/asymmetric ciphers in the following
way:
\begin{enumerate}
 \item a cipher $\Pi$ is symmetric if $K_\Enc$ and $K_\Dec$ are equivalent.
 \item a cipher $\Pi$ is asymmetric if $K_\Enc$ and $K_\Dec$ are not equivalent.
\end{enumerate}
...
\item $\Dec$: Similarly, performing the above relabeling, we get
  ...
  \]
\end{itemize}
Now, checking the correctness of $\Pi_{shift}$: Let $k, m \in \Int_n$
\[
...
\end{quotation}
This principle will not only be reflected in the design of ciphers, but also in
our proofs of security, where we will always assume that the adversary knows
everything about the mechanism of the cipher.
...
the adversary who wishes to break our scheme. We start with the analysis of the
``simple'' case, that is, the security of symmetric ciphers.
\paragraph{}
Since we would like to talk about what the adversary's chances are of breaking
the scheme, the language of probability theory is the most natural to use in the
formalisation of our ideas. The probabilistic aspect of what message, key or
ciphertext we are talking about can then be neatly captured in random variables.
Concretely, from now on let $M$ denote a random variable with some arbitrary
distribution over $\M$. Similarly, let $K$ be the random variable with some
arbitrary distribution over $\K$. Finally, We could simply say that the random
variable $C$ should fix a distribution over $\C$, however, we can do better than
that. Since all ciphertexts are completely determined by the encryption
algorithm, the messages and the possible keys, the distribution of $C$ should be
also completely determined. In particular, we shall define $C = \Enc_K(M)$, i.e.
the encryption algorithm over the randomness of $K$ and $M$.
...
We will also impose a very stringent constraint on what we
consider secure: we will require that an adversary observing our communications
\textbf{cannot gain any information} whatsoever. Formally:
...
\begin{proof}
  ...
  as required.
\end{proof}
The above definition is very compact, but it does not explicitly mention how an
adversary would be involved in an attack, so perhaps it's not the best way of
defining security after all. Therefore, we now approach the problem from a much
more hands-on perspective: we describe a concrete attack on the scheme in terms
a \textbf{security experiment}:
\paragraph{}
...
\item Put $k \leftarrow \Gen()$ and $c \leftarrow \Enc_k(m_b)$.
\item $\A$ is given $c$, and outputs $b' \in \{0, 1\}$
...
\end{enumerate}
\paragraph{Note:} Since we will be using experiments quite a lot in this work,
we shall refer to $B$ as a uniformly distributed random variable over $\{0, 1\}$
in the experiment.
In the above attack, the adversary $\A$ is asked to tell apart the encryptions of
two messages. Clearly, an adversary that is able to \textit{distinguish} two
ciphertexts is capable of learning some information about the underlying
messages, so this is precisely what we wish to disallow. Formally:
...
\]
\end{definition}
Now that we have two different notions of security, the question is which one
should we use? As it turns out, it does not matter, as the two definitions
...
\begin{proof}
  ...
  $f_2(C)$ are also independent\footnote{See theorem \ref{thm:func_independence} in the appendix}. Since $\A$ is just an arbitrary
  ...
  \end{align*}
  as required.\\
  `$\Leftarrow$' We shall prove this direction through contraposition. Thus,
  ...
  \begin{algorithmic}
    ...
    \If{$c = \hat{c}$}{ \Return 0 }\Else{ \Return $b' \leftarrow \{0, 1\}$
      chosen uniformly }\EndIf
    \State
  \end{algorithmic}
  ...
  \end{align*}
  Focusing on $m_0$ for now, by the definition of $C$, we have
  \begin{align*}
    \Prob(\A(C) = 0 \,\,|\,\, M = m_0) &= \Prob(\A(c) = 0 \,\,|\,\, C = c \land M = m_0) \\
                                   &=\sum_{c\in C} \Prob(\A(c) = 0 \,\,|\,\, C = c)\cdot \Prob(C = c\,\,|\,\, M = m_0) \\
                                   &=\sum_{\substack{c\in C \\ c \neq \hat{c}}} \Prob(\A(c) = 0 \,\,|\,\, C = c) \cdot \Prob(C = c\,\,|\,\, M = m_0) + \\
                                   &\quad\quad\quad + \Prob(\A(c) = 0 \,\,|\,\, C = \hat{c}) \cdot \Prob(C = \hat{c}\,\,|\,\, M = m_0)
  \end{align*}
  By the definition of $\A$, $ \Prob(\A(c) = 0 \,\,|\,\, C = \hat{c}) = 1$, and
  similarly $ \Prob(\A(c) = 0 \,\,|\,\, C = c) = \frac12$ for $c \neq \hat{c}$. Hence,
  we have
  \begin{align*}
    \Prob(\A(C) = 0 \,\,|\,\, M = m_0) &= \sum_{\substack{c\in C \\ c \neq \hat{c}}} \frac12 \Prob(C = c\,\,|\,\, M = m_0) + \Prob(C = \hat{c} \,\, |\,\, M = m_0) \\
                                   &=\frac12 \sum_{c\in C} \Prob(C = c \,\,|\,\, M = m_0) + \frac12\Prob(C = \hat{c} \,\,|\,\, M = m_0) \\
                                   &= \frac12 + \frac12\Prob(C = \hat{c} \,\,|\,\, M = m_0). \\
  \end{align*}
  Similarly, we will get
  \[
    \Prob(\A(C) = 1 \,\,|\,\, M = m_1) = \frac12 - \frac12\Prob(C = \hat{c} \,\,|\,\, M = m_1). 
  \]
...
\paragraph{}
Information theoretical security definitons, although guaranteeing defence
...
desirable to relax our notions of security to something more ``realistic''.
This will allow us to design much more efficient schemes.
...
\begin{definition}{Negligible function:}
  Let $f: \Nat \to \Reals$ be a function. Then, we say that $f$ is
  \textbf{negligible} iff $\forall p: \Nat \to \Reals$ polynomials $\exists
...
\end{definition}
A useful property of negligible functions is that they are closed under addition
and polynomial multiplication, i.e. if $f, g$ are negligible functions and $p, q$
are arbitrary polynomials, then
\[
  p(n) \times f(n) + q(n) \times g(n)
\]
is also negligible. See Proposition 3.6 in \cite{katz2014introduction} for a proof.
...
Naor\footnotemark. We will consider so-called \textbf{goals} (our definition of a scheme being broken) and \textbf{threat} or
...
algorithms since by the Extended Church-Turing Thesis, any ``efficient'' or ``reasonable'' model
...
algorithm, we also allow it to fail with negligible probability, and we will
...
take advantage of certain properties of the system to increase their chances of
...
could be used to reliably predict what was encrypted. This definition will be
familiar since we have already seen its information-theoretic equivalent in
Definition \ref{def:perfect_indistinguishability}. This is intuitively what probably
...
\begin{definition}{IND-atk:}
  ...
  security parameter $\lambda$, attack model atk and adversary $\A$ is defined as follows:
  \begin{enumerate}
  ...
  \end{enumerate}
\end{definition}
\paragraph{Reminder:} We shall denote the independent choice of the bit $b$ as the random
variable $B$.
\paragraph{Note:} The adversary is allowed to ``remember'' the results from its
first stage in its second stage. (The adversary is \textit{stateful}.)
...
plaintext $m$ of some ciphertext $c$, but is content with just modifying it to
obtain another ciphertext $\hat{c}$, such that when decrypted,
$\Dec_{sk}(\hat{c})$ is related to $m$ in some way. The classic example here is
...
recipient. Imagine an adversary observing this communication, and being able to
...
\begin{definition}{NM-ATK:}
  ...
  be giving credit to $\A$ for copying the ciphertext and outputting the equality relation.
...
\paragraph{} For public key cryptosystems, CPA is the most basic model.
...
\begin{definition}{Encryption Oracle:}
  ...
  $\Enc$ is probabilistic, it uses ``fresh'' randomness during every query (uses
  i.i.d. random variables for every query).
\end{definition}
From the above, the definition of CPA is straightforward.
...
\begin{definition}{CCA2 Experiment:}
  ...
  $\A$ may not query the decryption oracle on the challenge ciphertext. The
  reason for the restriction is that if we allowed the attacker to decrypt the
  challenge, then security in this framework would be clearly impossible.
\end{definition}
...
notions of security where the goal is fixed, only the threat model is different.
...
\begin{proof}
  ...
  \[
    \Adv{IND-atk}{\Pi, \A'}(n) \leq \negl(n).
  \]
  To do just that, we define an NM-ATK adversary $\A$, that will make use of $\A'$.
  ...
  model ATK. Below $\bar{x}$ denotes the binary complement of $x$, i.e.
  $\bar{x}_i = 1 - x_i$ where $x_i$ is the $i$-th bit of $x$.
  \begin{algorithmic}
    ...
    \State
  \end{algorithmic}
  Below, we will use the fact that without the loss of generality, we may assume
  ...
  This means that $\A$ succeeding in case 1 is exactly as likely as $\A'$ succeeding.
  Looking at the above definition of $\A$ this should seem fairly obvious, and
  we formally verify this intuition here. First, assuming that in the
  NM-Experiment we had $\A$ output $M = \{x_0, x_1\}, x \leftarrow M, c
  \leftarrow \Enc_{pk}(x), b' \leftarrow \A'(c)$ and $(c', R) \leftarrow \A(c)$
  with $x' = \Dec_{sk}(c')$.

  \paragraph{} Note that $R(x, x') = True$ if and only if $\Dec_{sk}(c) = x_{b'}$. That is, $\A'$
  ...
  always have $x_0 \neq x_1$, and hence we observe that $\Dec_{sk}(c) =
...
\begin{proof}
  ...
  \begin{algorithmic}
    ...
    \State $\vec{x}' \leftarrow \Dec_{sk}(\vec{c}')\quad$ (can do this by CCA2, and
    $c \not\in \vec{c}'$ since $\A'$ is an NM adversary)
  ...
  and $\A$ guesses $0$ randomly. Hence, $p_n(b)$ and $p'_n(b)$ are related in the
  following way:
  \[
    p_n(b) = \frac12 (1 + p'_n(b))
  \]
  where the first term comes from the coin flip $(\frac12)$ and the second term
  ...
  to $\A'$ and it then $R$-relating it to $x_0$ is precisely $\A'$
...
strongness towards the arrow's target (i.e. $A \rightarrow B$ means $A$ implies
...
``horizontal'' arrows, for the remaining proofs, please see Section 3 in \cite{bellaresecurityrelations}.
\begin{figure}
  \centering
  \begin{tikzpicture}
...
\begin{proof}
  ...
  threat model, since we are assuming that the public key and $\Enc$ are known
...
unrealistic to only consider this simple case. Luckily, in the case of
asymmetric encryption, it can be shown that \textit{goal-atk}
...
only acknowledge the result and not prove it. For a rigorous treatment see
Section 11 in \cite{katz2014introduction} (in particular Theorem 11.6).
\section{Public-Key Crypto Schemes}
\paragraph{} When one first encounters the concept of ciphers it is tempting to believe that
the only way of obfuscated information exchange may only work is if the method that
decrypts some text is the ``inverse'' of the method that was used to encrypt it.
The classic analogy here is that a cipher is a ``chest'' into which the honest
parties can place their information and ``lock'' it and ``unlock'' it with the
same key. Indeed, this general belief was held up until quite recently, until the
...
wishes to send a message could still place the message in the chest, however, they
would lock it with a ``padlock '' (the public key) and only the receiving party would
have the key to the padlock (the private key). As mentioned earlier, formally
this means that $\K_\Enc$ is not equivalent to $\K_\Dec$. Next, we will discuss
some problems that allow us to create such systems and then
...
\textbf{trapdoors} or \textbf{trapdoor functions}. The idea then is to use the
...
\begin{definition}{The Factoring Experiment:} The factoring experiment
  $\Expr{factor}{\A, \GenModulus}(n)$ given a security parameter $\lambda =
  1^n$, involving a $\PPT$ adversary $\A$ and $\PPT$ algorithm $\GenModulus$ is as follows:
  \begin{enumerate}
...
The RSA experiment $\Expr{RSA-inv}{\A, \GenRSA}(n)$ involving a $\PPT$ adversary $\A$ and
  ...
  \end{enumerate}
\end{definition}
\paragraph{} Informally, the RSA experiment asks the adversary to find an $e$-th
root of $y$ in $\Int_N$.
...
assumption, meaning we can rely on the hardness assumption of a much better-studied problem. To do this, we will now show how to obtain the algorithm
...
factoring assumption does, and there is a hard-to-invert exponent $e$!
The great thing about the RSA assumption is that it will give us a really nice
trapdoor, that we will present in the next part of this section.
\paragraph{Note:} Choosing $e$ can be quite tricky in practice. Here we just
mention that this is possible.
\subsection{The RSA crypto scheme}
...
is usually denoted $\phi(n) = |\Int_n|$ and is called the \textbf{Euler phi-function}.
...
a security parameter $\lambda = 1^n$. Increasing it will make the scheme run
...
$\GenRSA$, inverting $f$ is hard. However, if $d = e^{-1} \mod N$ is known,
...
\[
  \forall (pk, sk) \in \K, \forall m \in \M\quad\text{we have}\quad
  \Dec_{sk}(\Enc_{pk}(m)) = m.
\]
Assume we are given $pk = (N, i), sk = (N, j)$.
\[
  c = \Enc_{pk}(m) = m^i \mod N.
\]
Decrypting, we get
\begin{align*}
  m' &= \Dec_{sk}(c) \\
...
RSA'' since it presents the cipher's underlying mathematical mechanism in its purest form,
...
construction is insecure in every security framework we discussed. This issue has been first
...
\paragraph{}
At this point, we start to finally focus on the main topic of the dissertation:
homomorphic schemes. Intuitively, these encapsulate the idea of 
computation over encrypted data, in the sense that the adversary may at most
observe the operation take place, but both the inputs and the output of it
remain secret. In this section, we give the formal definition and construct an
example scheme known as the Paillier scheme. Finally, we close with a discussion
on the limitations of homomorphic schemes.
\subsection{What is homomorphic encryption?}
First, we see whence these schemes got their name:
\begin{definition}{(Group) Homomorphism:}
  Let $G$ and $H$ be groups. Then, a function $f: G \to H$ is called a
  homomorphism, iff $\forall g \in G, h \in H$ we have
  \[
    f(gh) = f(g)f(h).
  \]
  If $f$ is a bijection, we say that $G$ and $H$ are \textbf{isomorphic} and
  write
  \[
    G \cong H.
  \]
\end{definition}
Informally, homomorphisms are structure-preserving maps, and if two groups are
isomorphic, then they are virtually the same object. Now, to put this in the
context of ciphers:
\begin{definition}{(Group) Homomorphic Cipher:}
  Let $\Pi = (\Gen, \Enc, \Dec)$ be a cipher with message space $\M$ and cipher
  space $\C$. Furthermore, let $(\M, \circ)$ and $(\C, \oplus)$ be groups where
  $\circ, \oplus$ are binary operations.
  We say that $\Pi$ is homomorphic iff $\forall (pk, sk) \in \K, \forall
  m_1, m_2 \in \M, c_1 = \Enc_{pk}(m_1), c_2 = \Enc_{pk}(m_2)$ we have
  \[
    m_1 \circ m_2 = \Dec_{sk}(c_1 \oplus c_2).
  \]
  More compactly, the above says that $\Pi$ is homomorphic is $\Dec_{sk}(\cdot)$ is a
  homomorphism for all $sk$.
\end{definition}
...
result is achieved by Yasuda et al. \cite{Yasuda2018}, where they construct a
cipher where in fact the relationship between the message and the cipher space is
...
\cite{paillier1999public}. Once again, before we discuss the
...
\end{theorem}
\begin{proof}
  See Appendix, theorem \ref{app:phiformula}
\end{proof}
\begin{theorem}
  \label{thm:paillierthms}
Let $\lambda = 1^n$ be our security parameter. Let $p, q$ be primes such that
...
In particular, the isomorphism $f:\Int_{N} \times \Int_{N}^* \to \Int_{N^2}^*$
is given by
\[
...
Then clearly $\gcd(p, p - 1) = 1$ and $\gcd(p, q - 1) = 1$. If $\gcd(q, p - 1)
\neq 1$, then $\gcd(q, p - 1) = q$, since $q$ is a prime. But then $p - 1 = aq$
for some $a \geq 2$, in particular $\frac{p - 1}{2} \geq q$ from which $|p| >
...
N)^N = 1$, and this is clearly the smallest such positive integer.
\paragraph{Claim 3:}
...
\begin{proof}
  ...
  $a_1 = a_2$.
  \paragraph{} Going back to our initial formula, now simplifying the terms
  involving the $a$s:
  \[
    b_1^N = b_2^N \mod N^2.
  \]
...
\begin{definition}{Nth residue:} Let $G$ be a group and let $x, y \in G$. Then, we call $y$
  an Nth residue, if $x^N = y$, and write $y \in \texttt{Res}(G, N)$.
\end{definition}
We now characterise Nth residues in $\Int_{N^2}^*$.
\begin{theorem}
  Let $x \in \texttt{Res}(\Int_{N^2}^*, N)$ be an Nth residue. Then, $\exists b \in
  \Int_N^*$ such that
  \[
  ...
  and thus
  \[
    \texttt{Res}(\Int_{N^2}^*, N) = \{b \,\,|\,\, b \in \Int_N^*\}.
  \]
\end{theorem}
\begin{proof}
...
for $a$. This means sampling $\Int_{N^2}^*$ uniformly will allow us to uniformly
sample $\texttt{Res}(\Int_{N^2}^*, N)$ uniformly too!
\paragraph{} Finally, in this section we consider a new hardness assumption:
   \begin{definition}{The Decisional Composite Residuosity Problem (DCRP):} The
     ...
     we have
     \[
       \Prob(\A(N, [r^N \mod N^2]) = 1) - \Prob(\A(N, r) = 1)
       \leq \negl(n)
     \]
...
\mod N^2$. Then, calculate $m' = \frac{(\hat{c} - 1)}{N} \cdot \phi(N)^{-1} \mod
N$.
Output $m'$.
\paragraph{Note:} the division by $N$ happens in $\Rat$ and not in $\Int_N$!
...
\begin{align*}
  \hat{c} &= c^{\phi(N)} = (m, r)^{\phi(N)} = (\phi(N)m, r^{\phi(N)}) \\
          ...
          &\leftrightarrow (1 + N)^{\phi(N)m} \mod N^2 \\
          &= (1 + \phi(N)mN) \mod N^2\quad \text{by Theorem \ref{thm:paillierthms} part 2}.
\end{align*}
Then, we have
\begin{align*}
  \Dec_{sk}(c) &= \frac{\hat{c} - 1}{N} \cdot \phi(N)^{-1} \mod N\\
               &= \frac{(1 + \phi(N)mN) - 1}{N} \cdot \phi(N)^{-1} \mod N\\
               &= \frac{\phi(N)mN}{N}\cdot \phi(N)^{-1} \mod N\\
               &= \phi(N)m \cdot \phi(N)^{-1} \mod N \\
               &= m \mod N. \\
               &= m \quad \text{ since } m < N.
\end{align*}
Thus, $m = \Dec_{sk}(\Enc_{pk}(m))$, and so the scheme is correct, and therefore
a cipher.
...
\[
  \Prob(\A'(N, [r^N \mod N^2]) = 1) - \Prob(\A'(N, r) = 1) \leq \negl(n).
\]
Substituting the two above case analyses we get
\[
\Prob(\Expr{IND-CPA}{\Pi, \A} = 1) - \frac12 \leq \negl(n)
\]
and hence
\[
...
computation on encrypted data. In this section, we aim to provide a brief introduction
...
\begin{definition}{Vector Space:}
  ...
  vector space (under vector addition and scalar multiplication) iff for all $\vec{v}_1, \vec{v}_2 \in V$ and $\alpha_1, \alpha_2
  \in \Reals$ we have
  \[
...
\begin{definition}{Linear Combination:}
  Let $V$ be a vector space, and $B = \{\vec{b}_1, \hdots, \vec{b}_m\} \subseteq
  V$. Then, a linear combination of the vectors in $B$ is the vector
  \[
    \vec{b} = \alpha_1\vec{b}_1 + \hdots + \alpha_m\vec{b}_m\quad\text{ with }
    \alpha_1, \hdots \alpha_m \in \Reals.
  \]
  The set of all combinations that can be made up using the vectors in $B$ is
  called the \textbf{span} of $B$. Formally:
  \[
    \Span(B) = \{\vec{b} = \alpha_1\vec{b}_1 + \hdots + \alpha_m\vec{b}_m
    \,\,|\,\, \alpha_1, \hdots \alpha_m \in \Reals, \vec{b}_1,\hdots, \vec{b}_m
    \in B \}.
  \]
\end{definition}
\paragraph{} One can think of the span of a set of vectors as the ``smallest''
space in which all vectors related to the elements of $B$ can be found.
Geometrically, it is the ``smallest'' set of points that can be reached from the
origin by translating by the scaled version of the vectors in $B$. It is usually
convenient to collect vectors in $B$ in a structured way, called a matrix. If we
collected the scaling factors in a vector $\vec{\alpha} = [\alpha_1, \hdots,
\alpha_m]$, then we can express the vector $\vec{w}$ by writing
\[
  \vec{b} = \vec{\alpha} \cdot B = \vec{\alpha} \cdot 
  \begin{bmatrix}
    - \vec{b}_1 - \\
    \vdots \\
    - \vec{b}_m - 
...
\begin{definition}{Linear Independence:}
  Let $V$ be a vector space, and $B = \{\vec{b}_1, \hdots, \vec{b}_m\} \subseteq
  V$. We say that the vectors of $B$ are linearly independent if
  \[
    \vec{0} = \alpha \cdot B\quad\text{ with }
    \vec{\alpha} = [\alpha_1, \hdots \alpha_m] \in \Reals^m 
 \]
  holds if and only if $\alpha_1 = \alpha_2 = \hdots = \alpha_m = 0$. If the
  condition does not hold, we call the vectors of $B$ linearly dependent.
\end{definition}
\paragraph{} Geometrically, one can think of the span of linearly independent vectors as
the set of points that can be ``reached'' in a unique way through the vectors
of $B$, i.e. every point is defined by a unique linear combination. This
motivates our next definition:
\begin{definition}{Basis:}
  Let $V$ be a vector space, and $B = \{\vec{b}_1, \hdots, \vec{b}_m\} \subseteq
  V$. Then, we say $B$ is a basis for $V$ if we have $\Span(B) = V$ and the
...
\begin{definition}{General Linear Group:}
  It turns out that that the set of $n \times n$ invertible matrices (over a
  ring $R$) form a group under matrix
  multiplication. This group is called the general linear group and is usually
  denoted $\GL_n(R)$.
\end{definition}
\paragraph{} We now briefly discuss the idea of projecting a vector onto
another, as well as measuring the length of a vector and the distance between two
vectors.
...
\begin{definition}{Norm:}
  ...
  \]
  The norm of a matrix $B$ can then be defined to be the norm of its
  largest-normed row:
  \[
    \norm{B} = \sup_i\{\norm{\vec{b}_i} \,\,|\,\,\vec{b}_i \in B\}
  \]
\end{definition}
Finally, using the definition of length, we may measure the distance between any two
vectors by measuring the length of their difference:
\begin{definition}{Distance between two vectors (Metric):}
...
\begin{definition}{Orthogonality:}
  ...
  \]
\end{definition}
We will use one more notion related to distances later, namely that of a ball:
\begin{definition}{Ball:}
  Let $V$ be a vector space, $\vec{v} \in V$ and $r \in \Reals$. Then the
  (closed) ball centred around $\vec{v}$ is defined as
  \[
    B(\vec{v}, r) := \{\vec{w} \in V \,\,|\,\, \norm{\vec{w} - \vec{v}} \leq r\}.
  \]
\end{definition}
\subsubsection{Rings}
...
\begin{definition}{Field:}
  Let $(F, +, \times)$ be a ring. If $(F \setminus \{0\}, \times)$ forms a
...
\]
where $r_0,\hdots, r_m \in R$ are the coefficients of $p$. We call $x$ the
\textit{indeterminate} and the $x^i$
the \textit{powers} of $x$. The highest power of $x$ is the \textit{degree} of the
polynomial. The set of all finite degree polynomials over a ring $R$ is dentored $R[x]$.
...
\[
  \forall p(x) = \sum_{i = 0}^n p_ix^i, q(x) = \sum_{i = 0}^m q_ix^i \in
  R[x]\quad p(x) \times q(x) = \sum_{i = 0}^{m + n} c_ix^i
\]
where
\[
...
\]
\end{definition}
Next, we consider some concepts which arise from the number theoretic origins
of rings.
\begin{definition}{Unit:}
  Let $R$ be a ring, that has a multiplicative identity. Then, we say that an
  element $u \in R\setminus\{0\}$ is a unit, iff it has a multiplicative inverse in $R\setminus\{0\}$
\end{definition}
It is easy to see that the set of units in a ring form a group called the \textit{group
of units} and is usually denoted $R^*$.
...
\begin{definition}{Irreducibility:}
  ...
  is irreducible if it is non-zero, is not a unit and all of its divisor pairs have a unit in them. In other
  words, if we have $r = uv$ for some $u, v \in R$, then either $u$ or $v$ is a unit.
\end{definition}
Finally, very similarly to group homomorphisms, we define ring homomorphisms:
\begin{definition}{Ring Homomorphism:}
  Let $R, S$ be rings. Then a function $f: R \to S$ is a homomorphism iff
  ...
  \[
    f(xy) = f(x)f(y).
  \]
  If $f$ is a bijection too, then we say that $R$ and $S$ are isomorphic and
  write
  \[
    R \cong S.
  \]
...
\begin{theorem}
  Let $\L$ be a lattice with basis $B$. Then $\F_B$ tessellates $\Reals^n$
...
\begin{proof}
  ...
  \frac12$ for all $i$. Then, letting $\ell = \sum_{i = 1}^n \alpha_i\vec{v}_i$ and $\vec{f}
  = \sum_{i = 1}^n t_i\vec{v}_i$, we clearly have $\ell \in \L$ and $\vec{f} \in
  ...
  \begin{align*}
    x &= (\alpha_1 + t_1)\vec{v}_1 + \hdots (\alpha_n + t_n)\vec{v}_n \\
      ...
      &= \ell + \vec{f} \quad \text{ as required.}
  \end{align*}
\end{proof}
We can use the above result to make the following observation: given a point in
...
\begin{theorem}
  ...
  Let $\L$ be a lattice with basis $B_1$. Then $B_2$ is also a basis for $\L$ if
  and only if, there exists a matrix $A$ with $\det(A) = \pm 1$ such that $AB_1 = B_2$.
\end{theorem}
\begin{proof}
  Let us take a change of basis matrix $A$, such that $AB_1 = B_2$. Then, since
  we are staying in the same lattice, $A$ must have integer entries and since it
  is a non-degenerative transformation it must have $det(A) \neq 0$. Then:
  \[
    1 = \det(I) = \det(AA^{-1}) = \det(A)\cdot \det(A^{-1}).
  \]
  Now, using the fact that $A$ has integer entries, we deduce that $\det(A)$ is
  an integer also, and hence $\det(A) = \det(A^{-1}) = \pm 1$. On the other hand,
  if $\det(A) = \pm 1$, then since $A^{-1} = \frac{1}{\det(A)}\cdot
  \text{adj}(A)$ and the adjugate of $A$ will clearly have integer entries,
  and hence so must $A^{-1}$. (See \cite{cohen1993course} pp. 54 for the definition of
  the adjugate matrix.)
\end{proof}
...
\]
\end{theorem}
Thinking geometrically, the theorem is stating something very intuitive: given
the set of all $n$-dimensional parallelepipeds, the one that has the largest
volume will be the $n$-dimensional hyperrectangle. For a proof, see Corollary
2.5.5 in \cite{cohen1993course}.
...
We now approach the final object that we will
...
\begin{theorem}{Ideal membership in integer polynomial fields:}
 Let $f \in \Int[x]$ be a (monic) irreducible polynomial of degree $n$. Let $R = \Int[x]/f$. Then, an
 element $p(x) = p_0 + \hdots + p_{n - 1}x^{n - 1}\in R$ is a polynomial of
 degree at most $n - 1$, and hence it may be
 represented by a vector $\vec{p} = [p_0, \hdots p_{n - 1}] \in \Int^n$. Define the \textbf{rotation
   basis} of $p$ by
 \[
   \mathcal{P} := \{\vec{p}_i \text{ the is the coefficient vector of }
   p(x)x^i \mod f(x) \,\,|\,\, i \in \{0, 1, \hdots, n - 1\}\}.
 \]
...
\end{theorem}
Before we get into the proof, we examine why rotation bases should be relevant
to us. Let $p, q$ as above, and recall polynomial multiplication:
\[
  p(x) \times q(x) = \sum_{i = 0}^{m + n} c_ix^i \mod f(x)\quad\text{ where }
  c_i = \sum_{\substack{0 \leq k, l \leq m + n \\ k + l = i}} p_kq_l.
\]
This can be rewritten as
\begin{align*}
  p(x) \times q(x) &= \left[ \sum_{i = 0}^{n-1} q_ix^i \cdot p(x)  \right]\mod f(x) \\
                   &= \sum_{i = 0}^{n-1} \left[ q_ix^i \cdot p(x) \mod f(x) \right] \quad\text{ we can now take out }q_i\\
                   &= \sum_{i = 0}^{n-1} q_i \cdot \left[p(x)x^i\mod f(x) \right] \quad\text{ switching to coefficient vectors now, } \\
                   &\Leftrightarrow \sum_{i = 0}^{n-1} q_i \cdot \vec{p_i} = \vec{q} \times P.\\
\end{align*}
This means that we can calculate the product of two polynomials in $R$ by
switching to their vector representation, multiplying one by the other's
rotation basis and switching back! With this insight, the proof is very straightforward.
\begin{proof} $q(x) \in (p(x)) \Rightarrow \vec{q} \in \L(P)$.\\
  $q(x) \in (p(x))$ means that there is some $v(x) \in R$ such that $q(x) = v(x)
  \times p(x)$. Now switching to the vector context, we have
  \[
    q(x) = v(x) \times p(x) \Leftrightarrow \vec{q} = \vec{v} \times P
  \]
  Since $\vec{v}$ is an integer vector, $\vec{v} \times P \in \L(P)$. Hence $\vec{q} \in \L(P)$.
\paragraph{} We observe that all of the above steps are reversible, hence the
...
\paragraph{} Finally, in this section, we draw attention to the two modding operations that we
...
\paragraph{} How easy is it to find the shortest vector in a randomly picked,
$n$-dimensional lattice? And what should it even mean to find the shortest
vector? We answer these questions below in reverse order.

\begin{definition}{The $\gamma(n)$-SVP Experiment:} The Shortest Vector Problem experiment
  ...
  \end{enumerate}
\end{definition}
\paragraph{} Then, in the hope that it is easy to find a ``hard'' lattice, we
can define the hardness assumption of this problem.
\begin{definition}{$\gamma(n)$-Shortest Vector Problem (SVP) Assumption:}
  ...
  \]
\end{definition}
\paragraph{} A similar and related problem is, informally, given a point
$\vec{v}$ from the lattice's ambient space, finding the closest lattice point to
$\vec{v}$. We formalise this below:
\begin{definition}{The $\gamma(n)$-CVP Experiment:} The Closest Vector Problem experiment
  ...
  \end{enumerate}
\end{definition}
\paragraph{} Again, in the hope that we can find ``hard'' lattice bases, we can
define the appropriate hardness assumption:
\begin{definition}{$\gamma(n)$-Closest Vector Problem (CVP) Assumption:}
  ...
  \]
\end{definition}
\paragraph{} Now that we know what we mean by ``finding'' vectors, we can answer
our second question above: are there $\PPT$ algorithms $\GenBasis$ and $\GenCVP$ as specified in the above
definitions, with respect to which our hardness assumptions hold? A related
question is, how are CVP and SVP connected? It would be preferable to find a
connection so that we can base the hardness
assumption of one problem on the assumption of the other (as we did with the
factoring and RSA assumptions in Section \ref{sec:hardness_assumptions}). Ajtai shows in
\cite{AjtaiLatticeAvgWorst} that solving average-case (i.e. randomly
...
CVP and SVP, they can both be reduced to each other with the lattice dimension
...
on $\GenBasis$ also.
\paragraph{} An interesting different approach relies on the idea of modding a
...
named B\'abai's algorithm in his honour. What is great about this approach, is that
...
encoded using the public basis and can be recovered using the secret basis. The
scheme called GGH that we introduce at the end of this section will be a
straightforward application of this approach. In fact, a slightly twisted
version of the above ideas will give us the scheme in the last section.
...
\begin{definition}{The Ideal Membership Experiment:} The ideal membership experiment
  ...
  \end{enumerate}
\end{definition}
Informally, we are asking that given a ring $R$ and $I$ and some sampling function
...
\paragraph{Example}
Recall Paillier's scheme, in particular, that it relies on the isomorphism
\[
  \Int_{N^2}^* \cong \Int_N^* \times \Int_N
\]
where $N = pq$ for $p, q$ primes of equal length. Recall, that the set of $N$th
residues $\texttt{Res}(\Int_{N^2}^*,N)$ is the set of $N$th powers in $\Int_{N^2}^*$. Fix $a
\in \Int_{N^2}$. Then,
\begin{align*}
  (a + N)^N &= a^N + \binom{N}{1}a^{N-1}\cdot N + \hdots \text{ terms with a factor of } N^2 \\
            ...
            &= a^N \mod N^2.
\end{align*}
This means that $a^N = (a \mod N)^N$, thus there is a bijection between
$\texttt{Res}(\Int_{N^2}, N) \leftrightarrows \Int_N$. Then,
since both $\Int_N$ and the additive group of $(N)$ are cyclic and have the same
number of elements, they are isomorphic. Thus,
\[
  \texttt{Res}(\Int_{N^2}, N) \leftrightarrows \Int_{N} \cong (N).
\]
Hence, solving the IMP in $\Int_{N^2}$ for the ideal $(N)$
is equivalent to solving the DCRP for $\Int_{N^2}$ and $\texttt{Res}(\Int_{N^2}, N)$!
\begin{definition}{Ideal Membership Problem (IMP) Assumption:}
  ...
  \[
    \Adv{IMP}{\A, R, I, \Samp_I}(n) = 2 \cdot \Prob(\Expr{IMP}{\A, R, I, \Samp_I}(n) = 1) - 1 \leq \negl(n)
  \]
\end{definition}
...
$V \in \GL_n(\Int)$ with small Hadamard ratio. Then, generate a random matrix
$U \in \GL_n(\Int)$ such that $|\det(U)| = 1$. Calculate $W = UV$. Output
$(pk, sk) = (W, V)$.
\paragraph{Encryption}
Let $pk = W$ and $m \in \Int^n$ be the message that we want to encrypt. Let $r
\in \Int^n$ be a random vector with $\norm{r}$ small. Then, calculate and output
$c = mW + r$.
\paragraph{Decryption}
...
\paragraph{} The IND-CPA security of GGH relies on the result that CVP can be transformed into SVP and
that SVP can be transformed into its decisional version \cite{cheng_svp_red},
i.e. given a lattice basis $B$ and a real number $r > 0$, the adversary needs
to decide if $\lambda_1(\L(B)) < r$ or not. Then, using this version of SVP, the
theorem is very similar to Theorem \ref{thm:gentry_ind_cpa} and so we omit
it here.
\paragraph{} We also note that when GGH was introduced, the effectiveness of
sophisticated \textit{lattice basis reduction algorithms} (such as LLL) was not
considered. As it turns out, these algorithms can effectively break sufficiently
large keys, that it is impractical to use GGH (\cite{hoffstein2014introduction},
Section 6.7).
...
such scheme: the PhD thesis of C. Gentry \cite{gentry2009fully}.
...
attempting to present the whole work would take a much longer work than this
...
concrete, then implemented cipher. 
...
be as quick as possible, whilst retaining security (here we are talking about
asymptotic performance). In our case, as the reader
...
parallelisable (\cite{hoffstein2014introduction}, Section 6.7).
...
the ring homomorphisms between the message and the cipher space that we need to
achieve FHE.
...
different problem of the ideal coset problem (ICP), which will yield us a nice
trapdoor. Relying on the ICP, we will encode the message into an offset from
a lattice point. 

\paragraph{} So how will all the above come together to give us FHE?
Here is the abstract construction in 3 (broad) steps:
\paragraph{Step 1:} We first define a cipher that will allow computation 
\textit{up to a certain level} (through a ring homomorphism). This limitation will come
from the fact that the ``error'' associated with or ciphertext (the
coset representative) in which we embed our messages will increase as we combine
ciphertexts. Adding and multiplying ciphertexts will increase the error associated with their sum/
...
combination of operations so that it is capable of \textit{computing its own
  decryption function} (augmented with some operation at the end)!
The term we will use to describe a cipher with such a property is that
...
we wish to evaluate).
\paragraph{}
Here is the core of the construction: Let $\Pi = (\Gen,
...
\begin{enumerate}
  ...
  \item Calculate $c_1 = \Evaluate(\Dec, \{sk_1\, c_0\})$, i.e. we homomorphically
...
to be able to evaluate its decryption function \textbf{together with some
  operation}. Hence, in the second step we can actually call this augmented function
$\Dec^{op}$ with more than one ciphertext and make progress at the same time as
resetting the error.
\paragraph{Step 3:} Instead of using different public-private key pairs, we
just use one pair! In particular, we will encrypt twice under the same public
key and then homomorphically decrypt using the encryption of the associated
secret key. Is it safe to encrypt the secret key under its own private key? This
and other technicalities, unfortunately, fall out of the scope of our work,
however, we note that under some mild assumptions there are appropriate
workarounds and the scheme will be secure.

\paragraph{} In the upcoming few sections, we will mainly focus on the first two
steps. We will present the scheme that is
...
simulate any Turing Machine. Then, by the Church-Turing Thesis, we have that we can
compute anything that is computable. We will now clarify some terminology:
\begin{definition}{Circuit:} Let $\Gamma$ be a set of $k$-ary operations (also
  called \textbf{gates} in this context) over some set $S$. Then, an (acyclic) circuit is a tree
  with root $o$, where all nodes are gates in $\Gamma$ and edge connecting gate
  $g_1$ on level $n$ with gate $g_2$ on level $n - 1$ indicates that the output
  of $g_2$ is going to be an input to gate $g_1$. The leaves of the tree are
  called the inputs of the circuit. If we substitute in a value for every leaf,
  then calculate the value of every node and finally arrive at the value of the
  root node $o$, we say that the circuit was \textbf{evaluated}, and the result
  is the output of $o$.
\end{definition}
\begin{definition}{Functionally complete set:}
  Let $\Gamma$ be a set of boolean (binary) gates. Then, $\Gamma$ is functionally complete, if every Boolean
...
we can base our correctness and security proofs on the abstract scheme. This means that if we
can ``substitute in'' more concrete objects that satisfy our assumptions, we
will not need to worry about correctness or security!
...
We make the last assumption because we know that eventually, we would like to
work with lattices. Then, based on the trapdoor provided by the ICP, we define
the following scheme:
\paragraph{Key Generation} We are given a security parameter $\lambda = 1^n$. We
generate (a description of) a ring $R$ appropriate to $\lambda$. Also fix an ideal
...
circuit $\sigma \in \Sigma_\Pi$ for some permitted set of circuits $\Sigma_\Pi$ (see
...
ideals $I, J$ output by key generation. Then, it is enough to show that $R/J$
has a ring homomorphism to a functionally complete set of gates. In our concrete
case we will see that
\[
  (R/J, +_J, \times_J) \cong (\{0, 1\}, \land, \oplus),
\]
where $\oplus$ denotes the $\texttt{XOR}$ operation. Then, if $R = S[\vec{x}]$ is a (multivariate)
polynomial ring (which it will be), then there is a one-to-one correspondence
...
\begin{definition}{Generalised circuit:}
  Let $R$ and $I$ be as in our cipher and $\sigma$ be a mod-$I$ circuit. Then, the \textbf{generalised circuit} $g(\sigma)$ of
  $\sigma$ is formed by lifting the mod-I operations of $\sigma$ to the operations of $R$.
\end{definition}
\begin{definition}
  $X_\Enc$ be the image of $\Samp_R$. Then, all ciphertexts output by $\Enc$
...
\end{definition}
\begin{definition}{Permitted Circuits:}
\label{def:perm_circuits}
  Let
  \[
    \Sigma'_\Pi = \{\sigma \,\,|\,\, \forall \vec{x} \in X_\Enc^t \,\,\text{ we have }\,\,
    g(\sigma)(\vec{x}) \in X_\Dec\}.
  \]
...
\begin{definition}{Valid Ciphertext:}
  We say that a ciphertext $c$ is \textbf{valid} with respect to $\Pi$, public
  key $pk$ and permitted circuits $\Sigma_\Pi$ iff there exists some $\sigma \in \Sigma_\Pi$
  and $\vec{c} \in \C^t, t \in \Nat$ such that
  \[
    c = \Evaluate(pk, \sigma, \vec{c}).
  \]
  (Note, that $\sigma$ may be the identity circuit, i.e. the circuit that simply
  returns its input.)
\end{definition}
...
\begin{proof}
  ...
  evaluation of a permitted circuit $\sigma \in \Sigma_\Pi$, we get
  \[
    \Evaluate(pk, \sigma, \vec{c}) = g(\sigma)(\vec{c}) \mod J^{pk} \in g(\sigma)(m_1 + i_1,
    \hdots m_t + i_t) + J.
  \]
  Now, using the fact that $\sigma \in \Sigma_\Pi$, we know that the result of the lifted circuit
  $g(\sigma)$ can be correctly decrypted (i.e. $g(\sigma)(X_\Enc, \hdots, X_\Enc) \in X_\Dec$). Formally:
  \begin{align*}
    \Dec(sk, \Evaluate(pk, \sigma, \vec{c})) &= g(\sigma)(m_1 + i_1,
                                          \hdots m_t + i_t) \mod I \\
                                        &= g(\sigma)(m_1, \hdots m_t) \mod I\quad\text{due to the lifting property} \\
                                        &= \sigma(m_1, \hdots, m_t).\quad \text{as required by correctness.}
  \end{align*}
\end{proof}
\subsubsection{Security of the construction}
\paragraph{} Finally, in this section, we prove the security of our scheme.
\begin{theorem}
  \label{thm:gentry_ind_cpa}
...
\begin{proof}
  ...
  \begin{algorithmic}
    ...
    \State
  \end{algorithmic}
  \paragraph{} The intuition behind the above definition is that if $\A$ is any good at
  guessing $\beta$ given a non-uniformly sampled $t$,
  then $\beta' \oplus \beta$ will be 0 more often than it will
  be 1. Then, by our hardness assumption the problem cannot
  be solved with reliable success, and so there is a limit on how good $\A$
  can be at guessing in any case. Below we formally analyse the behaviour of the above algorithm:
  ...
  elements of $\texttt{Res}(\Int_{N^2}, N)$, here the elements of $(s) + J$ such as
  $t \times s$ will map onto $J$ in some $k$-to-$1$ fashion). Thus, the
  ciphertext will be uniformly random in $R/J$ and will be independent of
  $\beta$. Hence $\A$ will have 0 advantage (i.e. cannot
...
implementation, while exposing the pure mathematical machinery that
...
The reason is not only because of their decryption complexity is lower than known
...
\[
  \Sigma_\Pi' = \{\sigma\,\,|\,\, \forall \vec{c} \in X_\Enc^t\,\,\text{ we
    have }\,\,g(\sigma)(\vec{c}) \in X_\Dec\}.
\]
...
\paragraph{} The first ``substitution'' into the abstract scheme will
...
implementation, however, it is not necessary for the concrete description). Note
...
\begin{definition}{Encryption and Decryption Radii:}
  Let $r_\Enc$ be the smallest value, such that $X_\Enc \subseteq B(r_\Enc)$,
  where $B(r)$ is the ball of radius $r$ centred at the origin. Similarly, let
  $r_\Dec$ be the largest value such that $B(r_\Dec) \subseteq X_\Dec$. Then, let
  \[
    \Sigma_\Pi = \{\sigma\,\,|\,\,\forall \vec{c} \in B(r_\Enc)\,\,\text{ we have
    }\,\,g(\sigma)(\vec{c}) \in B(r_\Dec)\}.
  \]
...
\paragraph{} Given $r_\Enc$ and $r_\Dec$ we can now ask a more precise question:
what is the size of $\Sigma_\Pi$? To answer this, we need to bound the length of
the output of a generalised circuit. Namely, given $\sigma \in \Sigma_\Pi$ and $\vec{c}
\in B(r_\Enc)$, what can we say about $\norm{g(\sigma)(\vec{c})}$? Since the
...
\begin{definition}{Circuit Depth:}
  Let $\sigma = (E, V)$ be an (acyclic) circuit represented as a tree, with root node
  $o$. Define the set of level $l$ gates by 
  \[
    \sigma^{l} = \{v \in V\,\,|\,\, d(v, o) = l\}.
  \]
  Now, define the depth of $\sigma$ denoted by $\delta(\sigma)$ by
  \[
    \delta(\sigma) = \max_{i \in \Nat} \left\{ \sigma^{(i)} \neq \emptyset \right\}.
  \]
\end{definition}
Note, that according to our definition $\sigma^{(0)}$ is simply the singleton set
containing the root of the tree, and $\delta(\sigma)$
...
1\} \times \{0, 1\} \to \{0, 1\}$. We generalise this notion slightly further
...
circuit depths as low as possible. Hence, where possible, we can replace
a $\log_2k$-deep complete binary tree-circuit (which is the most efficient
circuit we can have to evaluate $k$ binary gates) of addition gates that with a $k$
fan-in addition gate, which has depth $1$.
We are now ready to state the theorem on the circuit bounds:
\begin{theorem}
  Assume $r_\Enc \geq 1$. Given a circuit $\sigma$, assume its $\Add$ gates have
  fan-in at most $\gamma(R)$ and its $\Mult$ gates have fan-in 2.
  Finally, assume that
  \[
    \delta(\sigma) \leq \log_2\log_2 r_\Dec - \log_2\log_2(\gamma(R) \cdot r_\Enc).
  \]
  Then, $\sigma(\vec{c}) \in B(r_\Dec)$ for all $\vec{c} \in B(r_\Enc)$, and hence $\sigma
  \in \Sigma_\Pi$.
\end{theorem}
\begin{proof}
  Let $r_i$ be an upper bound on the norm of the results of gates in $\sigma^{(i)}$
  for $0 \leq i \leq \delta(\sigma)$. Then, our task is to bound $\norm{g(\sigma)(\vec{c}}
  \leq r_0$ by $r_\Enc$, the radius of the input nodes in the circuit. Hence, $r_{\delta(\sigma)} = r_\Enc$. Then
  an $\Add$ gate at level $i$ will take as input at most $\gamma(R)$ number
  ...
  to $r_0$ $\delta(\sigma)$ times, we get:
  \[
    r_0 \leq (\gamma(R)\cdot r_\Enc)^{2^{\delta(\sigma)}}
  \]
  Taking logs twice then:
  \begin{align*}
    \log_2\log_2r_0 &\leq \delta(\sigma) + \log_2\log_2(\gamma(R)\cdot r_\Enc)\\
  ...
  \[
    r_0 \leq r_\Dec,
  \]
  and so $g(\sigma)(\vec{c}) \in B(r_\Dec)$.
\end{proof}
\paragraph{} We now see that besides minimising $r_\Enc$ and maximising $r_\Dec$
we should also minimise $\gamma(R)$. However, beyond the fact that we would like to
...
$f$ that is both irreducible and has a favourable expansion factor. For a more elaborate
...
criteria when considering the choice of ring, we would like to continue
...
write $V, W$ for their rotation bases, respectively. Then, substituting into
our abstract scheme, our decryption function becomes
\[
...
decrypted in the fundamental domain of $V$. Geometrically, $r_\Dec$ is
...
\paragraph{} To stay consistent with the notation in literature, we remind the
reader that for integers $c, d$ we shall write $[c]_d$ to refer to the
\textit{centred mod} of $c$ by $d$. This operation will work precisely as the
...
Normal Form of an integer matrix. We define it here so that we avoid
clutter later.
...
lattice (we will expand on this in a moment). For a treatment of
the HNF, we refer the reader to Section 2.4.2 in \cite{cohen1993course}.
The reason why we will want to use the HNF is that it always exists, can be
...
using $f(x) = x^n + 1$, not only because it is irreducible\footnote{See
  Appendix, Theorem \ref{thm:f_irred}}, but also as we
...
Due to the simplicity of $f$, $v$'s rotation basis has a special form (because
modding by $f$ essentially becomes setting all $x^n$ terms to $-1$): 
\[
   V =
   \begin{bmatrix}
   ...
   \end{bmatrix}
 \]
We now calculate the HNF of $V$ and check if it is of the form:
\[
  H = \begin{bmatrix}
    d & 0 & 0 &\hdots &0 \\
    -r & 1 & 0 &\hdots &0 \\
...
that it would allow us to store it very compactly: we would only need to
...
contains the row 
\[
  \vec{h}_2 = \vec{y} \times V = [-r, 1, \hdots, 0]
\]
for some $\vec{y} \in \Int^n$. Then, multiplying both sides by $W$, we get
\begin{gather*}
  \vec{y} \times V \times W = [-r, 1, \hdots, 0] \times W \\
  \vec{y} \cdot d = -r \cdot [w_0, w_1, \hdots, w_{n - 1}] + [-w_{n - 1}, w_0, \hdots, w_{n-2}]. 
\end{gather*}
Now, we can mod the whole equation by $d$ and get
\begin{gather*}
  r \cdot [w_0, w_1, \hdots, w_{n - 1}] = [-w_{n - 1}, w_0, \hdots, w_{n-2}] \mod d.
\end{gather*}
Due to linear independence now, we have $rw_{i+1} = w_i$ for all $0
\leq i \leq n - 2$ and $rw_0 = -w_{n - 1}$. Then, multiplying all these terms
together, we get
\[
  r^n\cdot w_0w_1\cdots w_{n - 1} = -w_{n - 1}w_0\cdots w_{n - 1} \mod d.
\]
...
$-1$ factors. Hence, it is enough to check for the last equation to see that
we have the HNF in the required form (up to an even number of negated rows). For
a proof of the existence of a row $\vec{h}_2$ being a sufficient condition for
...
public key $J^{pk}$. Then to encode the message as the representative of a coset
...
taking its centred mod by $d$, so it will fall in the interval $[-d/2, d/2)$.
...
\[
  \vec{c} = \vec{c}' - [ 0 , 2u_1,\hdots 2u_{n-1}] \times H = [[2\cdot
  u(r) + b]_d, 0, \hdots, 0]
\]
...
\begin{definition}{Compact Homomorphic Encryption:}
  Let $\Pi = (\Gen, \Enc, \Dec, \Evaluate)$ be a homomorphic cipher. Then, we say that it is \textbf{compact},
  iff for every security parameter $\lambda$ there is a (family of) permitted circuits $D_\Pi$
  expressing $\Dec$ of depth $\Oh(\lambda)$.\\
  Further, we say that if $\Evaluate$ is correct for some set of circuits $\Sigma_\Pi$ too,
...
\begin{definition}{Fully Homomorphic Encryption:}
  ...
  \textit{all circuits}.
\end{definition}
\begin{definition}{Bootstrappable Scheme:}
  ...
  \] 
\end{definition}
\begin{definition}{Levelled Fully Homomorphic Encryption:}
...
\end{definition}
\paragraph{} In this section, we will show (a sketch of the proof) that we can construct a levelled
fully homomorphic scheme from any bootstrappable scheme $\Pi$. We note, that
setting up the scheme, proving its correctness and its security requires some
...
of the next level, $pk_{i-1}$. Now, every ciphertext is doubly encrypted, once
...
Step 2 we
can now \textit{homomorphically decrypt} the ciphertext, resetting the error and
...
\Enc_{pk}(sk))$! Thus, we have finally arrived at FHE.
...
\paragraph{} As mentioned in the introduction, there are four \texttt{Sage}
implementations of the ciphers presented in this report, along with four
walkthroughs in the form of \texttt{Jupyter Notebooks} accompanying this text. 
They are supposed to help to understand the concepts of
their respective cryptosystems, by providing a bridge between theory and
implementation. Here, we briefly describe the design decisions that we took when
creating these.
\subsection{Tools}
\paragraph{} We opted for the \texttt{Sage} language and the environment it
provides to implement the entirety of the project. The reason is twofold:
\begin{enumerate}
\item It implements basically all the abstract algebraic (and related)
 functionality that this work required with sufficient coherence and efficiency.
\item Due to its extreme expressiveness, it allowed us to present a very close
  approximation of the theoretical schemes without much boilerplate.
\end{enumerate} 
We also used \texttt{Jupyter Notebooks} to provide walkthroughs of the code
interleaved with relevant theory. These notebooks are great as they allowed us
to create brief and good quality narratives very easily. They also allow the use
of \texttt{Markdown} and a limited amount of in \TeX to be embedded in the document.
Finally, we used \url{https://www.draw.io/} to create a couple of diagrams to
further aid understanding.
\subsection{Structure}
\paragraph{} We attempted to stay consistent with the implementation patterns
for each of the cryptosystems as well as with the text. This should allow the
reader to cross-examine two ciphers or switch between the report and the
implementation fairly easily. Every file is made up of three core functions:
\texttt{key\_gen, encrypt} and \texttt{decrypt}, which correspond to the triple
$\Pi = (\Gen, \Enc, \Dec)$ in the text. Some of the ciphers required some extra
functionality, such as the calculation of the Hadamard ratio, these are also
present in their files. 
\paragraph{} We attempted to make these as self-contained as
possible, such that they would only rely on the \texttt{Sage} standard library.
The only place where this was not possible was the GGH implementation,
where for convenience we rely on the \texttt{MatrixSpace} extension, and in its
walkthrough, we use a cryptography extension to demonstrate a lattice basis
reduction attack on the scheme.
\paragraph{} As for the walkthroughs, we created their narratives so that they
match the descriptions of the ciphers in this report. Namely, we always begin
with a brief discussion of the hard computational problem that yields the
trapdoor upon which the scheme will be based. Then, we discuss key generation,
encryption and decryption. This is followed by a discussion on the correctness
of the scheme. The walkthroughs are usually closed by the exposition of some
property unique to the system and/or security considerations. In particular, the
reader can find a direct implementation of Theorem \ref{thm:determ_enc_insecure}
in the RSA walkthrough and an implementation of Theorem
\ref{thm:homom_enc_limit} in the Paillier walkthrough.
\paragraph{} In addition to their exposition, there are some topics mentioned
briefly in the notebooks that are not mentioned/ expanded upon in this report. These are:
\begin{itemize}
\item RSA: factorisation attacks for low moduli.
\item GGH: visual representation of the scheme and lattice reduction attack
  using LLL on a low-dimensional lattice.
\item Gentry's scheme: concrete example of converting a circuit to a
  multivariate polynomial and evaluating it.
\end{itemize}
...
\begin{proof}
  ...
  principle. Hence, let $\P$ be the set of prime divisors of $N$, and
  ...
  \[
    ...
    N - 1\}\,\,|\,\,p \text{ divides } n\,\,\forall p \in P\}.
  \]
 ...
 \sum_{\substack{P \subseteq \P \\ |P| = n - i}} \prod_{p \in P}p \right) \\
                   &=\prod_{i = 1}^n p_i^{e_i - 1}\prod_{p \in \P} (p - 1) \quad\text{by Equation \ref{eq:prodexpansion}}\\
                   &= \prod_{i = 1}^n p_i^{e_i - 1}(p_i - 1).
 \end{align*}
\end{proof}
