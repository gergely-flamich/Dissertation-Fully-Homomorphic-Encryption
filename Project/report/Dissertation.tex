...
\begin{definition}{NM-atk:}
  ...
  \begin{enumerate}
    \item $\A$ is given full knowledge of $\Pi$, and anything additional that
    ...
    \item $(pk, sk) \leftarrow \Gen(\lambda)$, and $m \leftarrow M$ are
      generated. $c = \Enc_{pk}(m)$ is calculated.
    \item $\A$ is given $c$ and anything additional that the attack model
    ...
    \item $\A$ succeeds if for $\textbf{m}' = \Dec_{sk}(\textbf{y}) = (\Dec_{sk}(y_1),
      \hdots,\Dec_{sk}(y_{t-1}))$ we have $R(m, \textbf{m}') = True$.
  \end{enumerate}
\end{definition}
...
\end{definition}
From this definition, the definition 
