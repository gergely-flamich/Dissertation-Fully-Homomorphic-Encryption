...
wish? The aim of this work is essentially to provide an answer to this question.
\paragraph{}
...
\paragraph{}
One of the earliest known cipher goes back at least 2000 years, to Roman times. We will now examine the
...
parctice the punctuation and spaces between words would also have been removed \cite{ceasarCipher}.
\paragraph{}
...
\begin{definition}{(Cipher)}
  ...
  A cipher (also called cryptoscheme or cryptosystem) $\Pi$ over $\M$, $\K_\Enc$ and $\K_\Dec$ is a tuple $(\Gen, \Enc, \Dec)$, where
  \begin{itemize}
...
more hands-on prespective: we describe a concrete attack on the scheme in terms
...
\begin{definition}{((Group) Homomorphic Cipher)}
  ...
  \]
  More compactly, the aboves says that $\Pi$ is homomorphic is $\Dec_{sk}(\cdot)$ is a
...
N$.
Output $m'$.
\paragraph{Note:} the division by $N$ happends in $\Rat$ and not in $\Int_N$!
...
origin by translating by scaled version of the vectors in $B$. It is usually
...
\begin{definition}{Ball}
  ...
  (closed) ball centered around $\vec{v}$ is defined as
  \[
...
\begin{definition}{Field:}
  Let $(F, +, \times)$ be a ring. If $(F \setminus \{0\}, \times)$ forms an
...
rotation basis and swithcing back! With this insight, the proof is very straightforward.
...
definitios, with respect to which our hardness assumptions hold? A related
...
i.e. given a lattice basis $B$ and an real number $r > 0$, the adversary needs
...
\paragraph{Step 1:} We first define a cipher that will be allow computation 
...
key and then homomophically decrypt using the encryption of the associated
...
\begin{definition}
  ...
  \[
    \delta(\sigma) = \max_{i \in \Nat} \sigma^{(i)} \neq \emptyset.
  \]
\end{definition}
...
$\log_2k$ deep complete binary tree-circuit of addition gates that with a $k$
...
decrypted in the fundamental domain of $V$. Geometrically therefore, $r_\Dec$ is
...
\end{gather*}
Due to linear independence now, we therefore have $rw_{i+1} = w_i$ for all $0
...
\begin{definition}{(Compact Homomorphic Encryption)}
  ...
  Further, we say that if $\Evaluate$ is correct for some set of circuts $\Sigma_\Pi$ too,
...
\paragraph{} As mentioned in the introduction, there are 4 \texttt{Sage}
accompanying this text and are supposed to help understanding the concepts of
