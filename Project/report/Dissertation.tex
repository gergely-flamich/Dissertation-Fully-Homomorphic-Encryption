...
theory that will help us establish the required notions.
\subsection{Mathematical Set-up}
\begin{definition}{Random Oracle}
\end{definition}
\begin{definition}{Probabilistic Polynomial-time Algorithm}
 Let $\A$ be an algorithm with access to a random oracle. Then, we say that $\A$
...
\paragraph{}
Ciphertext-only attacks are the most basic notions of modern-day security. As
noted in the previous chapter, we shall use experiment-based models, as later it
will allow us to prove the security of various schemes much easier. Therefore we
begin with the definition of the appropriate experiment.
\begin{definition}{Eavesdropping Experiment}
  Let $\Pi$ be a cipher and $\A$ be a \PPT algorithm. Then, the experiment is as
  follows:
  \begin{enumerate}
  \item $\A$ outputs two messages $m_0, m_1 \in \M$.
  \item A key $(k_\Enc, k_\Dec) \leftarrow \Gen$ is generated and a bit $b$ is chosen uniform
    randomly. $c \leftarrow \Enc_{k_\Enc}(m_b)$ is calculated and given to $\A$.
  \item $\A$ guesses a bit $b'$
  \item If $b' = b$ we say that $\A$ succeeds and write $\Expr{eav}{\Pi, \A} =
    1$, and $0$ otherwise.
  \end{enumerate}
\end{definition}
Now, we are ready to state our first computational security definition:
\begin{definition}{Eav-security}
  We say that a scheme $\Pi$ is Eav-secure, if any \PPT adversary $\A$ can only
  succeed negligably better than probability a half. Formally: $\forall \A$
  $\PPT$ algorithms, we have
  \[
    \Prob(\Expr{eav}{\Pi, \A} = 1) = \frac12
    + \negl(n).
  \]
\end{definition}
...
\begin{theorem}
Let $\lambda \in \Nat$ be our security parameter. Let $p, q$ be primes such that
$2^\lambda < p, q < 2^{\lambda + 1}$ and $N = pq$. Then,
\[
  \Int_{N^2}^* \cong \Int_{N} \times \Int_{N}^*.
\]
