...
\begin{definition}{Divisibility}
  ...
  $r$, iff there is some $t \in R$, such that $r = st$.
\end{definition}
\begin{definition}{Irreducibility}
  Let $R$ be a ring with a multiplicative identity. Then, we say that an element
  $r$ is irreducible if all of its divisor pairs have a unit in them. In other
...
on $\GenBasis$ also.
\begin{definition}{$\gamma(n)$-Bounded Distance Decoding Problem (BDDP)}
\end{definition}
...
\begin{definition}{The Ideal Membership Experiment} The ideal membership experiment
  $\Expr{IMP}{\A, \GenIdeal}(n)$ involving a $\PPT$ adversary $\A$ and
  $\PPT$ algorithm $\GenIdeal$ is as follows:
  \begin{enumerate}
  \item We run $\GenIdeal(1^n)$, obtaining the tuple $(I)$, with $P, S \in
    \GL_n(\Rat)$ such that there exists a unimodular matrix $U$ with integer
    coefficients (i.e. $U \in \GL_n(\Int)$ with $|\det(U)| = 1$) such that $US =
    P$ and $\vec{w} \in \Span(P)$.
  \item $\A$ is given $P$ and $\vec{w}$, and outputs some $\vec{v} \in \L(P)$.
...
\end{definition}
\begin{definition}{Ideal Coset Problem (ICP)}
\end{definition}
\cite{Babai1986}
...
one. Instead, our goal is to distill the core concepts and put them in the
...
each new ciphertext has some ``error'' associated with it in which we embed our
