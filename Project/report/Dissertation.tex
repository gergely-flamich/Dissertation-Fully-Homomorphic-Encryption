...
consider secure also. We will require that given any ciphertext and a uniformly
...
\item Pick a string $r$ such that $|r| = |c|$ uniformly randomly.
\item $\A$ is randomly given $r$ or $c$, and outputs $b'= 1$ if it believes that
  it was given $c$, and $b' = 0$ otherwise.
\item We say that $\A$ \textbf{succeeds} iff $b = b'$ and write
  $\texttt{Expr}^{\texttt{eav}}_{\Pi, \A} = 1$.
\end{enumerate}
Now, we are ready for our first definition of security:
\begin{definition}{Perfect Security}
We say that a scheme $\Pi$ is perfectly secure iff for all adversaries $\A$
\[
  \Prob(\texttt{Expr}^{\texttt{eav}}_{\Pi, \A} = 1) = \frac12.
\]
\end{definition}
