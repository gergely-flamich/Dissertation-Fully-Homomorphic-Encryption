...
\begin{proof}
  ...
  Therefore, the probability that $\A$ guesses $0$ correctly must be equal to when $\A$ guesses
  ...
  \begin{align*}
    \Prob(\A \, \text{outputs 0} | B = 0) &= \frac12 \sum_{\substack{c \in \C \\ c \neq c_0, c_1}} \Prob(M = m | C = c) + \Prob(M = m | C = c_0)\\
                                          &= \frac12 (\sum{c \in \C} \Prob(C = c) + \Prob(M = m | C = c_0) - \Prob(M = m | C = c_1)) \\
                                          &= \frac12 (1 + \delta) \\
...
\paragraph{}
Ciphertext-only attacks are the most basic notions of modern-day security.
